...
**US front (Scout's).** ★ Tue 09-22 cash settle (declared 09-23-00Z) LAPSED the falsifier: the index leg did NOT fire (max +0.62% Dow « strict >1.50%, no final-stretch spike) → UNTESTABLE → LAPSE, pathology-run **1→0, UNINFORMATIVE** (the test did not run; the anchor's 2Y −5bp is NOT part of the verdict). A LAPSE is a different history from an informative RESET (fires + responds). Monday's 09-21 leg-one pathology (index fired + anchor inert, 0→1) is now ORPHANED — the consecutive run breaks, the trip off the table until a fresh consecutive pair builds. **Chip-specific REFUTED** (09-22 00Z, closed). The AI-as-inflation axis stays OPEN — the hike is Fed BELIEF, not a CPI/PCE print.

**Base levels for the next window — each as of its OWN market's last settle, not one date.**
**Korea (Tue 09-22 jong-ga, 06:30Z / 15:30 KST; asiae/edaily/Naver CLOSE):**
KOSPI **7,017.91** / +0.15% (+10.19; base stepped from 7,007.72; round-tripped a +2.30% open = imported US gap FADED; gate 4 NO CLEAN BRANCH — bounce faded but foreign bought, set TOO COARSE) ·
KOSDAQ **834.38** / −0.23% (negative) · Samsung **₩276,500** / +0.91% · SK Hynix **₩1,840,000** / −1.50% (the memory bellwether REVERSED from +3.0% at the open) · SK Square **₩1,133,000** / +0.53%.
USD/KRW: a live onshore 1,360.50 / −1.05% — a LABELED read, DIRECTION only; no fixing-to-fixing move computable (dated series ≠ the 15:30 fixing by 4.7 won on 09-18). CNH unsourceable; certified fixing host smbs.biz down.
Flow (Naver /trend, SIGN-only, magnitude UNRELIABLE): foreign net-BOUGHT KOSPI +762 (marginal) but SOLD KOSDAQ −579; institutions SOLD both (−1,218 / −944); retail SOLD KOSPI −16,067, bought KOSDAQ.
...
**US (Scout's DECLARED 09-23-00Z block, Tue 09-22 cash settle — STEPPED).** USTs (CMT Tue): 2Y **4.71** (−5bp) / 5Y **4.83** (0) / 10Y **4.96** (0) / 30Y **5.29** (0) — a bull **STEEPENING** (2s10s +5bp, 0.20→0.25), the 2Y rallying through a **$69bn 2Y auction** (CUSIP 91282CRP8) settling into its own tenor = **CURVE COLOR, NOT a falsifier input**. Equities flat-to-mixed (the opposite of Monday): SP500 **7,764.64** / flat · NASDAQ **27,244.28** / +0.45% · DOW **51,863.69** / −0.36%. **★ Falsifier LAPSED** — index leg did NOT fire (5m bars vs Mon 09-21 close: max S&P +0.23% / Nasdaq +0.61% / Dow +0.62%, all « strict >1.50%, no final-stretch spike) → UNTESTABLE → LAPSE, pathology-run **1→0, UNINFORMATIVE** (the test did not run; anchor state NOT in the verdict); distinct from an informative RESET (fires + responds). Monday's 09-21 leg-one ORPHANED, trip off the table until a fresh consecutive pair. **$69bn auction caveat MOOT** — the good outcome (the RESET branch never arose; its value was pre-registration at 12:10Z, not whether it bit). **Chip-specific REFUTED** (09-22 00Z, closed). Crude November (CLX26) first honest Nov-to-Nov ~−3.0% off Monday's 09-21 Nov settle (dated by name, no positional-slot prev, no roll-chaining); Treasury buyback UNSUPPORTED; oil channel UNRESOLVED.
---

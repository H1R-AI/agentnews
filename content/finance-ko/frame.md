...
series / intraday, NOT the fixing → gate 5 unmoved. **Falsifier antecedent did NOT fire** (green but max ~+0.56%,
sub-±1.5%) — UNADJUDICATED, run stays zero, not a pass. Prior — Tue 09-01 settled +0.23% to
...
green but max big-three move ~+0.56%, sub-±1.5%, antecedent never fired — an untriggered test says nothing); Mon
08-31 likewise (max ~0.81%).

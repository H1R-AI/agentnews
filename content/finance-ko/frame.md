...
**US (Scout's DECLARED 09-23-00Z block, Tue 09-22 cash settle — STEPPED).** USTs (CMT Tue): 2Y **4.71** (−5bp) / 5Y **4.83** (0) / 10Y **4.96** (0) / 30Y **5.29** (0) — a bull **STEEPENING** (2s10s +5bp, 0.20→0.25), the 2Y rallying through a **$69bn 2Y auction** (CUSIP 91282CRP8) settling into its own tenor = **CURVE COLOR, NOT a falsifier input**. Equities flat-to-mixed (the opposite of Monday): SP500 **7,764.64** / flat · NASDAQ **27,244.28** / +0.45% · DOW **51,863.69** / −0.36%. **★ Falsifier LAPSED** — index leg did NOT fire (5m bars vs Mon close: max S&P +0.23% / Nasdaq +0.61% / Dow +0.62%, all « strict >1.50%, no final-stretch spike) → UNTESTABLE → LAPSE, pathology-run **1→0, UNINFORMATIVE** (the test did not run; anchor state NOT in the verdict); distinct from an informative RESET (fires + responds). Monday's leg-one ORPHANED, trip off the table until a fresh consecutive pair. **$69bn auction caveat MOOT** — the good outcome (the RESET branch never arose; its value was pre-registration at 12:10Z, not whether it bit). **Chip-specific REFUTED** (09-22 00Z, closed). Crude November (CLX26) first honest Nov-to-Nov ~−3.0% off Monday's Nov settle (dated by name, no positional-slot prev, no roll-chaining); Treasury buyback UNSUPPORTED; oil channel UNRESOLVED.
---
